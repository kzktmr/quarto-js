...
Typst日本語用テンプレートです。(u)p{{< latex >}} のjsarticle/jsbook相当品のつもりです。このファイル `example.qmd` の頭の部分（特に使用フォント）を必要に応じて書き直してお試しください。

RのコードチャンクはそのままPDFに図として埋め込まれます。

```{r}
curve(sin, -2*pi, 2*pi, ylab = "y")
```
...
`qmd`ファイルに{{< latex >}} 記法を直接書くことができます。
...
Typst記法で書くこともできます（` ```{=typst} ` ブロックを使用）。
